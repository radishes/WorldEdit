...
namespace WorldEdit.Commands
{
    ...
    public class MazeCommand : WECommand
    {
        ...
        public MazeCommand(int x, int y, int x2, int y2, int plr, int tunnelWidth, int wallWidth, int algorithm)
            : base(x, y, x2, y2, plr)
        {
        ...
        public override void Execute()
        {
            ...
            {
                case 0:
                    TShock.Players[plr].SendMessage(String.Format("Recursive Backtracker maze creation initiated!"), Color.Green);
        ...
        private void RecursiveBacktracker()
        {
            ...
            int size = this.tunnelWidth;
            int wallSize = this.wallWidth;
            ...
            while (cellStack.Count > 0)
            {
                ...
                foreach (int d in rndDirs)
                {
                    ...
                    { // a valid unvisited cell, begin excavation
                        ...
                        for (int i = p3.X; i < p3.X + offset.X; i++)
                        {
                            for (int j = p3.Y; j < p3.Y + offset.Y; j++)
                            {
                ...
                file.WriteLine(String.Format("Didn't like any directions; backtracking."));
